...
const Nominations: React.FC = () => {
  ...
  // Final diagnostic log as a string for easy reading
  console.log(`[NOMINATIONS_STATE] ${JSON.stringify({
    loading,
    userRole: user?.role,
    nominations: safeNominations.length,
    trainings: safeTrainings.length,
  ...
  try {
    ...
    return (
      <div className="space-y-6">
        {/* Diagnostic Overlay (Hidden by default, can be toggled via console if needed) */}
        <div id="debug-overlay" className="hidden fixed bottom-4 right-4 z-50 p-4 bg-background border border-primary/20 rounded-xl shadow-2xl font-mono text-[10px] text-primary">
          <p>NOMINATIONS: {safeNominations.length}</p>
          <p>TRAININGS: {safeTrainings.length}</p>
          ...
          <Card className="glass-card overflow-hidden">
            ...
            <CardContent className="pt-6 space-y-4">
              {pendingNominations.map((nomination) => {
                ...
                return (
                  <div
                    key={nomination.id || `pending-${nomination.participantId}-${nomination.trainingId}`}
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <h4 className="font-bold text-foreground tracking-wide text-lg">{participant?.name || 'Unknown Participant'}</h4>
                      <p className="text-xs text-muted-foreground font-mono mt-1 opacity-80 uppercase">
                        RANK: {participant?.designation || 'N/A'} • SECTOR: {getInstitutionName(nomination.institutionId)}
                      </p>
                      <div className="flex items-center gap-2 mt-3">
                        <div className="h-px flex-1 bg-border/50" />
                        <p className="text-[10px] font-bold text-primary tracking-widest uppercase">
                          ASSIGNED MISSION: {training?.title || 'Unknown Mission'}
                        </p>
                        <div className="h-px flex-1 bg-border/50" />
                      </div>
                    </div>
            ...
            {otherNominations.length === 0 ? (
              ...
              <div className="space-y-4">
                {otherNominations.map((nomination) => {
                  ...
                  return (
                    <div
                      key={nomination.id || `other-${nomination.participantId}-${nomination.trainingId}`}
                    ...
                    >
                      <div className="flex-1 min-w-0">
                        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                          <div>
                            <h4 className="font-bold text-foreground tracking-wide text-lg">{participant?.name || 'Unknown'}</h4>
                            <p className="text-xs text-muted-foreground font-mono mt-1 opacity-80 uppercase">
                              RANK: {participant?.designation || 'N/A'} • SECTOR: {getInstitutionName(nomination.institutionId)}
                            </p>
                          </div>
                        ...
                        <div className="mt-4 p-3 rounded-xl bg-primary/5 border border-primary/10">
                          <p className="text-xs font-bold text-primary tracking-widest uppercase">
                            MISSION: {training?.title || 'Unknown'}
                          </p>
                          {training && (
    ...
    return (
      <div className="p-8 text-center bg-destructive/10 border border-destructive/20 rounded-3xl m-6">
        <XCircle className="size-12 mx-auto mb-4 text-destructive" />
        <h2 className="text-xl font-bold text-destructive uppercase tracking-[0.2em]">MISSION CRITICAL FAILURE</h2>
        <p className="text-muted-foreground font-mono text-xs mt-2">The display logic encountered a fatal anomaly.</p>
        <div className="mt-6 p-4 bg-black/40 rounded-xl text-left font-mono text-[10px] text-destructive/80 overflow-auto">
          <p>ERROR: {err?.message || 'Unknown protocol error'}</p>
          <p className="mt-2 text-muted-foreground underline cursor-pointer" onClick={() => window.location.reload()}>RELOAD INTELLIGENCE FEED</p>
        </div>
      </div>
    );
  }
