...
import { Search, Trash2, AlertTriangle, Users, MapPin } from 'lucide-react';
...
type OfficerDirectoryProps = {
    roleFilter: Extract<UserRole, 'program_officer' | 'medical_officer'>;
    pageTitle: string;
    subtitle: string;
...
const OfficerDirectory: React.FC<OfficerDirectoryProps> = ({
    roleFilter,
    pageTitle,
    subtitle,
    searchPlaceholder,
    ...
    const filteredOfficers = useMemo(() => {
        ...
        return officers
            .filter((officer) => officer.role === roleFilter)
            .filter((officer) => {
                if (!normalizedTerm) return true;

                return (
                    (officer.name && officer.name.toLowerCase().includes(normalizedTerm)) ||
                    (officer.email && officer.email.toLowerCase().includes(normalizedTerm)) ||
                    (officer.department && officer.department.toLowerCase().includes(normalizedTerm)) ||
                    (officer.phone && officer.phone.includes(normalizedTerm))
                );
            });
    }, [officers, roleFilter, searchTerm]);
        ...
        <div className="space-y-8 text-foreground pb-12">
            ...
            <div className="space-y-4">
                <div className="flex items-center gap-3 px-1">
                    <h2 className="text-lg font-bold text-foreground/90">{pageTitle}</h2>
                    <span className="text-xs font-medium text-muted-foreground bg-secondary/20 px-2 py-0.5 rounded-full">{filteredOfficers.length}</span>
                </div>
